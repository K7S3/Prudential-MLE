...
def random_forest_regression(
    ...
    """
    Performs random forest regression with k-fold cross-validation.
...
def inference(
    model: RandomForestRegressor, test_data: pd.DataFrame, target: str
) -> None:
    """
...
def main():
    ...
    # Train the model
    model = random_forest_regression(train_data, target_variable)
    ...
    pd.DataFrame(y_pred, columns=["Predicted"]).to_csv(
        "../data/predicted/rf-data-1000.csv", index=False
    )
