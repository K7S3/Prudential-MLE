...
@given(
    st.integers(min_value=400, max_value=700), st.integers(min_value=100, max_value=300)
)
def test_calculate_bmi(height, weight):
    bmi = rules_engine.calculate_bmi(height, weight)
    assert isinstance(bmi, float)
    assert 0 <= bmi <= 100


@given(
    st.integers(min_value=18, max_value=70),
    st.floats(min_value=15.0, max_value=45.0),
    st.sampled_from(["Male", "Female"]),
)
def test_determine_quote(age, bmi, gender):
    quote, reason = rules_engine.determine_quote(age, bmi, gender)
...
def test_prepare_data():
    ...
    df_processed = rules_engine.prepare_data(df)
